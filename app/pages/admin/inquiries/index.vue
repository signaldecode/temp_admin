...
const fetchInquiryList = async () => {
  ...
  try {
    ...
    const response = await get('/admin/inquiries', params)

    inquiryList.value = response.data.content || []
...
const handleSelectAll = (selectAll) => {
  selectedIds.value = selectAll ? inquiryList.value.map((q) => q.id) : []
}
...
const bulkDelete = () => {
  ...
  // TODO: 벌크 삭제 API 연동
  inquiryList.value = inquiryList.value.filter((q) => !selectedIds.value.includes(q.id))
  selectedIds.value = []
  uiStore.showToast({ type: 'success', message: '삭제되었습니다.' })
}

// 답변대기 건수
const pendingCount = computed(() => inquiryList.value.filter((q) => q.status === 'PENDING').length)
  ...
  <LayoutListPage title="1:1 문의 관리">
    ...
    >
      ...
      <template #cell-userName="{ item }">
        <span class="text-sm text-neutral-600">{{ item.userName || '-' }}</span>
      </template>
